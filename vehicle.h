...
class Vehicle {
    public:
    Vehicle(cv::Mat& frame, const cv::Rect& loc);
    bool track(cv::Mat& frame);
    bool update_if_same(cv::Mat& frame, cv::Rect& new_loc, long long unsigned frame_count, bool reject_if_smaller = false);
    cv::Rect& get_loc();
    ...
    void init_tracker(cv::Mat& frame, const cv::Rect& loc);
